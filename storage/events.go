...
events(
    stream_ordering SERIAL PRIMARY KEY,
...
event_edges(
    ...
    prev_event_id TEXT,
    room_id TEXT,
    CONSTRAINT ee_uniqueness UNIQUE (event_id, prev_event_id, room_id)
);
...
state_power_levels(
    ...
    room_id TEXT NOT NULL,
    power_level INTEGER,
    CONSTRAINT spl_uniqueness UNIQUE (state_key, room_id)
);
...
room_id_depth(
    ...
    CONSTRAINT rid_uniqueness UNIQUE (room_id)
);
